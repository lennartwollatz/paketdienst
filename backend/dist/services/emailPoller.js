...
async function runAutoSync() {
    ...
    for (const user of users) {
        try {
            const { newOrders, mergedOrders } = await (0, emailAccounts_1.syncUserAccounts)(user.id);
            if (newOrders > 0 || mergedOrders > 0) {
                console.log(`[auto-sync] Nutzer ${user.id}: ${newOrders} neue, ${mergedOrders} zusammengeführte Bestellungen`);
            }
        }
        catch (err) {
